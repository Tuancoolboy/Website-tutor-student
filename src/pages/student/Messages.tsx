...
const Messages: React.FC = () => {
  ...
  const loadAvailableUsers = async () => {
    try {
      ...
      console.log('[Student Messages] Loading available users...')
      // Load all users (students, tutors, management) - không filter theo role
      const response = await usersAPI.list({ limit: 100 })
      console.log('[Student Messages] Users API response:', response)
      ...
      console.log('[Student Messages] Parsed users list:', usersList.length, 'users')
      ...
      if (usersList.length > 0) {
        ...
        console.log('[Student Messages] Filtered users (excluding current user):', filteredUsers.length, 'users')
